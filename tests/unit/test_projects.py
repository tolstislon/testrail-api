...
def test_get_project(api, mock, host):
    mock.add_callback(
        responses.GET,
        f'{host}index.php?/api/v2/get_project/1',
...
def test_get_projects(api, mock, host):
    mock.add_callback(
        responses.GET,
        f'{host}index.php?/api/v2/get_projects',
        get_projects,
    )
...
def test_add_project(api, mock, host):
    mock.add_callback(
        responses.POST,
        f'{host}index.php?/api/v2/add_project',
        add_project,
    )
...
def test_update_project(api, mock, host):
    mock.add_callback(
        responses.POST,
        f'{host}index.php?/api/v2/update_project/1',
        update_project,
    )
...
def test_delete_project(api, mock, host):
    mock.add_callback(
        responses.POST,
        f'{host}index.php?/api/v2/delete_project/1',
        lambda x: (200, {}, ''),
    )
