...
def add_result(r):
    ...
    return 200, {}, json.dumps(
        {'id': 1, 'status_id': data['status_id'], 'test_id': 15,
...
def test_add_result_for_case(api, mock, url):
    mock.add_callback(
        responses.POST,
        url('add_result_for_case/3/34'),
        add_result
    )
    resp = api.results.add_result_for_case(3, 34, status_id=1, comment='Passed',
                                           assignedto_id=1)
